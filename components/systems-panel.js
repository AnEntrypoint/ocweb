...
<div class="sp-add"><button class="sp-btn" id="btn-new-sys">+ New System</button></div>
  ...
  el.querySelector('#btn-new-sys').onclick = async () => { await fetchLayers(); openNewSysDialog(actor) }
}

function openNewSysDialog(actor) {
...
function openNewTermDialog(sysId, actor) {
  const overlay = document.createElement('div')
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:1000;display:flex;align-items:center;justify-content:center'
...
<button id="tc-ok" style="padding:6px 14px;font-size:13px;background:var(--primary);color:var(--primary-foreground);border:none;border-radius:6px;cursor:pointer">Open</button>
  ...
  overlay.querySelector('#tc-ok').onclick = () => {
    ...
    overlay.remove()
    const tid = uid()
    actor.send({ type: 'ADD_TERMINAL', systemId: sysId, terminal: { id: tid, label, cmd, wcId: uid() } })
    actor.send({ type: 'SELECT_TERMINAL', systemId: sysId, terminalId: tid })
  }
}
...
function renderMain(el, ctx, actor) {
  ...
  if (!sys) { el.innerHTML = '<div style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--muted-foreground);font-size:13px">No system selected</div>'; el._sysId = null; return }
  if (el._sysId !== sys.id) {
    el._sysId = sys.id
    el.innerHTML = `<style>
...
.sp-term-wrap{flex:1;min-height:0;position:relative}
  ...
  tabbar.querySelectorAll('[data-close]').forEach(b => b.onclick = (e) => {
    ...
    if (wcSys && sys.mode === 'ephemeral') {
      if (tabs.filter(t => t.id !== tid).length === 0) wcSys.destroy()
    }
  ...
  if (addBtn) addBtn.onclick = () => openNewTermDialog(sys.id, actor)
  if (!selTid) { termwrap.innerHTML = '<div style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--muted-foreground);font-size:13px">Open a terminal to begin</div>'; termwrap._tid = null; return }

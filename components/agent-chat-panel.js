...
import { setIframe, getCurrentUrl } from "../browser-tools.js";
...
function msgCtx(e, text, agentId, isUser, actor) {
  const items = [{ icon: iconHtml("clipboard", 14), label: "Copy text", action: () => navigator.clipboard.writeText(text).catch(() => {}) }];
  if (isUser) items.push({ icon: iconHtml("refresh", 14), label: "Retry message", action: () => runAgent(actor, agentId, text) });
...
function renderMsg(line, agentId, actor) {
  ...
  if (line.startsWith("tool: ")) return createElement("div", { class: "ui-command-surface", style: "padding:8px 12px;font-family:var(--font-mono);font-size:12px", innerHTML: iconHtml("wrench", 13) + " " + line.slice(6).replace(/</g, "&lt;") });
...
function render(actor, el) {
  ...
  const header = h("div", { style: "display:flex;align-items:center;gap:10px;padding:10px 16px;background:color-mix(in oklch,var(--surface-1) 92%,var(--surface-0));box-shadow:var(--shadow-2xs)" },
    ...
    h("button", { class: "ui-btn-icon", style: (showBrowser ? "color:var(--primary)" : ""), onclick: () => { showBrowser = !showBrowser; render(actor, el); } }, I("globe")),
    h("button", { class: "ui-btn-icon", onclick: () => actor.send({ type: "SHOW_PANEL", panel: "settings" }) }, I("settings")),
    h("button", { class: "ui-btn-icon", onclick: () => actor.send({ type: "SHOW_PANEL", panel: "brain" }) }, I("brain")));
  ...
  const chat = h("div", { style: "flex:1;display:flex;flex-direction:column;min-width:0" },
    h("div", { id: "oc-chat-scroll", style: "flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:10px" },
      ...
      agent.thinkingTrace ? h("div", { style: "max-width:68ch;padding:10px 14px;border-radius:12px;background:color-mix(in oklch,var(--primary) 8%,transparent);font-size:12px;font-style:italic;color:var(--muted-foreground);box-shadow:var(--shadow-2xs);display:flex;align-items:start;gap:6px" }, I("messageCircle", 13), agent.thinkingTrace) : null,
  ...
  const inputBar = h("div", { style: "display:flex;gap:8px;padding:10px 16px;background:color-mix(in oklch,var(--surface-1) 94%,var(--surface-0));box-shadow:0 -2px 8px rgba(0,0,0,.04)" },
    ...
    agent.status === "running"
      ? h("button", { class: "ui-btn-ghost", style: "color:var(--danger-soft-fg);font-size:12px", onclick: () => { abortAgent(agent.agentId); actor.send({ type: "UPDATE_AGENT", agentId: agent.agentId, patch: { status: "idle", streamText: null, thinkingTrace: null } }); } }, I("square"), " Stop")
      : h("button", { class: "ui-btn-primary", style: "font-size:12px", onclick: doSend }, I("send"), " Send"));
